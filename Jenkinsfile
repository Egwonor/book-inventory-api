    ...
    stage('Start Task on ECS Cluster') {
      steps{
        sh '''#!/bin/bash -x
          ...
          TASK_REVISION=`aws ecs describe-task-definition --task-definition new-ecs-fargate | egrep "revision" | tr "/" " " | awk '{print $2}' | sed 's/"$//'`
          echo $TASK_REVISION
          DESIRED_COUNT="1"
          
          aws ecs update-service --cluster new-ecs-fargate --service ${SERVICE_NAME} --task-definition ${TASK_FAMILY} --desired-count ${DESIRED_COUNT}'''
      }
    }
